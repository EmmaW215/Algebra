...
export default function Scene17PowersProductQuotientPractice({ onNext, onPrev }: { onNext: () => void, onPrev: () => void }) {
  const [selected, setSelected] = useState<string[]>([]);
  const [isCorrect, setIsCorrect] = useState(false);

  // Select 3 expressions equivalent to 4^10
  // A: (4^5)^2 = 4^10 (Correct)
  // B: 10*10*10*10 (Incorrect)
  // C: 4^20 / 4^10 = 4^10 (Correct)
  // D: 4^2 * 4^5 = 4^7 (Incorrect)
  // E: (4^2 * 4^3)^2 = (4^5)^2 = 4^10 (Correct)
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-8 w-full max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold text-slate-800">Practice: Equivalent Expressions</h2>
      
      <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        <p className="text-lg text-slate-600 font-bold">Select the <span className="text-purple-600">three</span> expressions equivalent to <span className="font-mono text-2xl">4¹⁰</span>.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <button onClick={() => toggleSelection("B")} className={`p-4 rounded-xl border-2 transition-all flex items-center justify-between ${selected.includes("B") ? "bg-purple-50 border-purple-500" : "border-slate-200 hover:border-purple-200"}`}>
            <span className="font-mono text-lg">10·10·10·10</span>
          ...
          <button onClick={() => toggleSelection("E")} className={`p-4 rounded-xl border-2 transition-all flex items-center justify-between col-span-1 md:col-span-2 ${selected.includes("E") ? "bg-purple-50 border-purple-500" : "border-slate-200 hover:border-purple-200"}`}>
            <span className="font-mono text-lg">(4² · 4³)²</span>
