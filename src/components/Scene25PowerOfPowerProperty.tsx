...
export default function Scene25PowerOfPowerProperty({ onNext, onPrev }: { onNext: () => void, onPrev: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-8 w-full max-w-4xl mx-auto">
      ...
      <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        <p className="text-lg text-slate-600 font-bold">Simplify. Rewrite in the form 8ⁿ.</p>
        ...
        <div className="flex items-center justify-center gap-2 text-4xl font-bold font-mono py-4">
          ...
          <sup className="flex items-center">
            <input 
              ...
              className="w-12 h-12 border-2 border-blue-500 rounded text-center text-xl focus:outline-none"
              placeholder="n"
            />
          </sup>
        </div>
        ...
        {isCorrect && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-green-600 font-bold space-y-2">
            <p>Correct!</p>
            <p className="text-sm text-slate-500">2 × 3 = 6</p>
          </motion.div>
        )}
      </div>
