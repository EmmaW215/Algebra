...
export default function Scene19QuotientPropertyPractice({ onNext, onPrev }: { onNext: () => void, onPrev: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-8 w-full max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold text-slate-800">Practice: Quotient of Powers</h2>
      
      <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        <p className="text-lg text-slate-600 font-bold">Solve for <span className="font-mono">x</span>.</p>
        ...
        {isCorrect && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-green-600 font-bold space-y-2">
            <p>Correct!</p>
            <p className="text-sm text-slate-500">7 - 3 = 4, so x = 4.</p>
          </motion.div>
        )}
      </div>
