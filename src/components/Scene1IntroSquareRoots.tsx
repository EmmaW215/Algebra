...
export default function Scene1IntroSquareRoots({ onNext, onPrev }: { onNext: () => void, onPrev: () => void }) {
  ...
  const checkStep1 = () => {
    if (inputValue === "3") {
      ...
      setFeedback("Try again. What number squared is 9?");
    }
  };

  const checkStep2 = () => {
    if (inputValue === "5") {
      setFeedback("Correct! The square root of 25 is 5.");
      setTimeout(() => {
        onNext();
      }, 1500);
    } else {
      setFeedback("Try again. What number squared is 25?");
    }
  };
    ...
    <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-8 w-full max-w-4xl mx-auto">
      ...
      <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6 min-h-[300px] flex flex-col justify-center">
        {step === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <p className="text-xl text-slate-600">
              We know that <span className="font-bold text-rose-600">squaring</span> a number means multiplying it by itself.
            </p>
            <div className="text-3xl font-mono font-bold text-slate-800">
              3² = 3 × 3 = <input 
                ...
                className="w-20 border-b-2 border-rose-500 text-center focus:outline-none"
                placeholder="?"
              />
            </div>
            <button onClick={checkStep0} className="px-6 py-2 bg-rose-600 text-white rounded-full font-bold hover:bg-rose-700">Check</button>
          </motion.div>
        )}
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <p className="text-xl text-slate-600">
              The <span className="font-bold text-rose-600">square root</span> goes the other way.
            </p>
            ...
            <button onClick={checkStep1} className="px-6 py-2 bg-rose-600 text-white rounded-full font-bold hover:bg-rose-700">Check</button>
          </motion.div>
        )}
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <p className="text-sm text-slate-500">"What number squared equals 25?"</p>
            <button onClick={checkStep2} className="px-6 py-2 bg-rose-600 text-white rounded-full font-bold hover:bg-rose-700">Check</button>
          </motion.div>
        )}
