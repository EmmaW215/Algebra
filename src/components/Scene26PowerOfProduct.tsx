...
export default function Scene26PowerOfProduct({ onNext, onPrev }: { onNext: () => void, onPrev: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center space-y-8 w-full max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold text-slate-800">Power of a Product Property</h2>
      
      <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        <p className="text-lg text-slate-600 font-bold">Simplify. Rewrite in the form 3ⁿ · 5ᵐ.</p>
        ...
        {isCorrect && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-green-600 font-bold space-y-2">
            <p>Correct!</p>
            <p className="text-sm text-slate-500">Distribute the exponent to each factor.</p>
          </motion.div>
        )}
      </div>
